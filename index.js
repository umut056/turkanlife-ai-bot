...
function parseContact(text) {
  ...
  const phoneMatch = text.match(/(5\d{2})[\s-]*\d{3}[\s-]*\d{2}[\s-]*\d{2}/);
...
bot.on("message", async (msg) => {
  ...
  if (s.stage === "await_contact") {
    ...
    sessions.delete(chatId); // Baştan başlasın
    return;
  }
});

console.log("Bot çalışıyor 🚀 (AI kapalı, sadece lead sistemi)");